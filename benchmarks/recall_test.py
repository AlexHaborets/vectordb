...
#     "collection", [{"dim": 128, "name": "sift_small", "distance": "l2"}], indirect=True
# )
...
# @pytest.mark.parametrize(
#     "collection", [{"dim": 128, "name": "sift", "distance": "l2"}], indirect=True
# )
...
def test_recall_glove(collection):
    print("\n[Init] Loading GloVe-100-Angular...")
    with load_dataset("glove") as (base, query, ground_truth):
        benchmark_recall(
...
def benchmark_recall(
    collection, base_vecs, query_vecs, ground_truth, expected_recall
) -> None:
    """
